...
// echo $td->textContent."\n";
		...
		if(($this->columns = fgetcsv($handle, 0, $this->separator, $this->enclosure, $this->escape)) === false)
			return false;
// _NSLog($this->separator);
// _NSLog($this->columns);
		$this->rows[]=array();
		while(($values = fgetcsv($handle, 0, $this->separator, $this->enclosure, $this->escape)) !== false)
			{
// print_r($values);
			$row=array();
...
// Alternative: DOMDocument aufbauen und dann schreiben
		...
		if(!fputcsv($handle, $this->columns, $this->separator, $this->enclosure, $this->escape, $this->eol))
			{
			fclose($handle);
			return false;
			}
		foreach($this->rows as $record)
			if(!fputcsv($handle, $record, $this->separator, $this->enclosure, $this->escape, $this->eol))
				{
				fclose($handle);
				return false;
				}
 ...
 */
	...
	public function fetchDict()	// alternate method name
	{ // fetch next row
		return mysqli_fetch_array($this->result);
	}

// FIXME: this is a wrongly named function
