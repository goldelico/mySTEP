...
- (void) mouseDown: (NSEvent *)theEvent
{
	...
	if (_clickedRow != -1
		...
		position += NSMinX([self rectOfColumn:_clickedColumn]);
...
- (void) drawRow: (NSInteger)rowIndex clipRect: (NSRect)aRect
{
	...
	x_pos = NSMinX (aRect);
	i = 0;
	while ((i < _numberOfColumns) && (x_pos > NSMinX([self rectOfColumn:i])))
		{
		i++;
		}
	startingColumn = (i - 1);
...
- (NSDragOperation) draggingUpdated: (id <NSDraggingInfo>) sender
{
	...
	if ((lastVerticalQuarterPosition != verticalQuarterPosition)
		...
		NSInteger pointerInsertionLevel = ceilf(0.5f * horizontalHalfPosition);
             ...
             select: (BOOL) flag
{
	...
	if ([_window fieldEditor: NO  forObject: self] != nil)
		{ // there is already a field editor
	...
	_editingCell = [[self preparedCellAtColumn: columnIndex row: rowIndex] copy];
	
	[_editingCell setEditable: _dataSource_editable];
	tb = [_tableColumns objectAtIndex: columnIndex];
	[_editingCell setObjectValue: [self _objectValueForTableColumn: tb
															   row: rowIndex]];
	
	// But of course the delegate can mess it up if it wants
	[self _willDisplayCell: _editingCell
			forTableColumn: tb
					   row: rowIndex];
	
	/* Please note the important point - calling stringValue normally
     causes the _editingCell to call the validateEditing method of its
	...
	t = [_editingCell setUpFieldEditorAttributes: t];
	// FIXME: Which background color do we want here?
	[t setBackgroundColor: [NSColor selectedControlColor]];
	...
	if (flag)
		{
		[_editingCell selectWithFrame: drawingRect
							   inView: self
							   editor: t
							 delegate: self
								start: 0
		...
		{
		[_editingCell editWithFrame: drawingRect
							 inView: self
							 editor: t
						   delegate: self
							  event: theEvent];
